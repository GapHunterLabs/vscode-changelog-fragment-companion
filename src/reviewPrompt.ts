 ...
 * a real number of distinct invalid changelog fragment file names flagged -- never on install, never on
...
const KEY_SEEN_FINDINGS = 'changelogFragmentCompanion.review.seenFindings';
const KEY_ANSWERED = 'changelogFragmentCompanion.review.answered';
...
function showPrompt(context: vscode.ExtensionContext): void {
  ...
  void vscode.window
    .showInformationMessage(
      `Changelog Fragment Companion: this has flagged ${HITS_BEFORE_PROMPT} real issues in your code -- if it's saved you time, a rating on the Marketplace helps other developers find it.`,
      rateAction,
      dismissAction,
    )
    .then((selection) => {
